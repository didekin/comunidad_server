...
import static com.didekin.incidservice.repository.IncidTables.INCID_COMMENT_TB;
import static com.didekin.incidservice.repository.IncidTables.INCID_COMU_TB;
...
import static com.didekin.incidservice.repository.IncidTables.INCID_RESOLUCION_AVANCE_TB;
import static com.didekin.incidservice.repository.IncidTables.INCID_RESOLUCION_TB;
...
enum IncidenciaSql {
    ...
    GET_INCID_BY_COMU("SELECT * FROM " + INCID_COMU_TB + " WHERE e_id = ?"),
    ...
    IS_INCID_OPEN("SELECT incid_id FROM " + INCID_COMU_TB +
            " WHERE incid_id = ? AND fecha_cierre IS NULL"),

    MODIFY_INCID_IMPORTANCIA("UPDATE " + INCID_IMPORTANCIA_TB + " as ii " +
            " left join " + INCID_COMU_TB + " as ic ON ii.incid_id = ic.incid_id" +
            " SET ii.importancia = ? " +
            " WHERE ii.incid_id = ? AND ii.e_id = ? AND ii.u_id = ? AND ic.fecha_cierre IS NULL"),

    MODIFY_INCIDENCIA("UPDATE " + INCID_COMU_TB + " SET descripcion = ?," +
            " ambito = ?" +
            " WHERE incid_id = ? AND fecha_cierre IS NULL"),

    MODIFY_RESOLUCION("UPDATE " + INCID_RESOLUCION_TB + " as ir " +
            " left join " + INCID_COMU_TB + " as ic ON ir.incid_id = ic.incid_id " +
            " SET ir.fecha_prevista = ?, " +
            " ir.coste_estimado = ? " +
    ...
    REG_INCID("insert into " + INCID_COMU_TB + " (incid_id, e_id, user_name, descripcion, ambito) VALUES (?,?,?,?,?)"),

    REG_INCID_COMMENT("INSERT INTO " + INCID_COMMENT_TB +
            " (incid_id, e_id, u_id, descripcion) " +
            "VALUES (?,?,?,?)"),

    REG_INCID_IMPORTANCIA("INSERT INTO " + INCID_IMPORTANCIA_TB +
            " (incid_id, e_id, u_id, importancia) VALUES (?,?,?,?)"),

    REG_RESOLUCION("INSERT INTO " + INCID_RESOLUCION_TB +
    ...
    SEE_INCIDS_CLOSED_BY_COMU("select " +
            ...
            " where i.e_id = ? and i.fecha_cierre IS NOT NULL and i.fecha_alta > DATE_SUB(NOW(), INTERVAL 2 YEAR) " +
    ...
    SEE_INCIDS_OPEN_BY_COMU("select " +
            ...
            " from " + IncidTables.INCID_USER_ALTA_VIEW + " AS i " +
            " LEFT join " + INCID_RESOLUCION_TB + " AS re " +
            " USING (incid_id) " +
            " LEFT join " + IncidTables.INCID_AVG_VIEW + " AS ivg " +
            " USING (incid_id) " +
            " where i.e_id = ? and i.fecha_cierre IS NULL " +
            " order by i.fecha_alta"),

    SEE_RESOLUCION("select * from " + INCID_RESOLUCION_TB + " where incid_id = ?"),
